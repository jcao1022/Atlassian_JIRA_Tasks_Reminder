...
if __name__ == '__main__':
    ...
    jql8 = 'project = CAFE AND issuetype not in (Task,Epic) AND Sprint in openSprints() and status not in (Resolved, Closed) ORDER BY assignee'

    if not jira.search_issues_by_jql(jql8):
        import os
        print "EEROR: No issues found!"
        os._exit(0)

    for i in  jira.search_issues_by_jql(jql8):
    ...
    for i in result:
        ...
        content += i['id'].encode('utf-8') +"    "+ i['assignee'].encode('utf-8') + "    " + i['issue_type'].encode('utf-8') + "    " + i['priority'].encode('utf-8') + "    " + i['link'].encode('utf-8') + "    " + i['summary'].encode('utf-8')
    ...
    for i in content.splitlines():
        ...
        for x in i.split("    "):
            ...
            if x == 'Bug':
                ...
                html += """ <td bgcolor="#cccccc" nowrap><font color="#006600"> %s </td>""" % x + '\n'
            else:
